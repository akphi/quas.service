...
let checkConnection = (callback) => {
  mongodb.MongoClient.connect(
    "mongodb://" + config.get('DB_MONGO_USER_USERNAME')
    + ":" + config.get('DB_MONGO_USER_PASSWORD')
    ...
    + "/" + config.get('DB_MONGO_DBNAME'), {
      ...
      if (error) {
        ...
        logger.info(serverMessage.DATABASE_CONNECTION_SUCCESS, config.get('DB_MONGO_HOST') + ":" + config.get('DB_MONGO_PORT') + "/" + config.get('DB_MONGO_DBNAME'));
        callback();
      }
    });
};
